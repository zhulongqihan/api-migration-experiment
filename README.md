...
**方向3 - 混合系统**：
- **数值稳定性修复**（2025-11-25）
 - 修复LLM生成时的`RuntimeError: probability tensor contains inf/nan`
 - 根本原因：Qwen2.5-Coder + transformers 4.44 + 采样模式数值溢出
 - 解决方案：全面采用greedy解码，避免repetition_penalty
- **策略优化**（2025-11-25）
 - 降低规则直接应用阈值：0.85 → 0.5
 - 扩大高准确率策略覆盖面
 - 预期精确匹配率提升至35-45%
...
**技术限制**：
- 采样模式在当前环境下不稳定（数值溢出）
- greedy解码导致规则引导策略效果受限
- 未来方向：升级transformers版本或更换模型
...
#### 3.2 方向1：LoRA微调（暂时搁置⏸，后续回归）

**最新更新**：2025-11-21 
**服务器路径**：`~/api_migration_exp/scripts/`
**状态**：⏸ **暂时搁置**（遇到深层兼容性问题，转向知识编辑方案）
...
#### 3.3 方向2：神经知识编辑（遇到技术障碍）

**最新更新**：2025-11-21 
**服务器路径**：`~/api_migration_exp/scripts/`
**状态**： **遇到技术障碍**（多次尝试失败，转向DPO方向）

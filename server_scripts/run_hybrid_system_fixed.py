...
def main():
    ...
    # 创建规则匹配器
    matcher = RuleMatcher(rules, confidence_threshold=config['confidence_threshold'])
    console.print(f"[green]✓ 规则匹配器就绪[/green]")
    
    # 创建混合生成器
    generator = HybridGenerator(matcher, model_name=config['model_name'], device=config['device'])
    ...
    console.print(Panel.fit(
        ...
        f"策略分布:\n"
        f"  • 规则直接: {strategy_stats.get('rule_direct', {}).get('count', 0)} 次\n"
        f"  • 规则引导: {strategy_stats.get('rule_guided', {}).get('count', 0)} 次\n"
        f"  • LLM兜底: {strategy_stats.get('llm_fallback', {}).get('count', 0)} 次\n\n"
        f"结果文件: {result_file}",
        border_style="green"
    ))
